...
| 7 | Rust unit tests | WinGet “already current” is accepted only when Node.js 24/npm actually run; transient platform failures retry; access denials do not retry; direct active child workspaces parse without eager app calls. | Native command execution is simulated through pure decision inputs. |
