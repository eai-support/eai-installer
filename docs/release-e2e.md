...
version, and the Ubuntu diagnostic contract requires EAI CLI `3.18.0` exactly
...
generated project. Both the checkpoint and final query encode the CLI 3.18.0
...
contract exercised by EAI CLI 3.18.0: the app and tenant must match the
...
`eai-cli` version pinned in `installer-manifest.json` (currently `3.18.0`), and
